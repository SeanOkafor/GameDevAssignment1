 ...
 * 
 * ATTACK SYSTEM:
 *   Each phase has 2 attacks. When IDLE, the boss randomly picks one via RNG.
 *   After an attack finishes, the boss returns to home position and picks
 *   the next attack randomly.
 * 
 * PHASE 1 ATTACKS:
 ...
 *   DEFEATED        → all 2 phases beaten, boss is gone
 * 
 * DAMAGE FLASH:
...
public class FirstBoss {
	...
	// Attack1 projectile display size (380×261 reduced by 75% → 95×65)
	...
	// --- Attack 2: Dive Bomb ---
	private enum Attack2SubState { SLIDE_OFF, SHOW_ARROWS, FIRE_ARROWS }
	...
	private static final int ATK2_ARROW_SPAWN_DELAY = 50;  // 0.5 seconds
	...
	private static final double ATK2_ARROW_SPEED = 8;
		...
		switch (atk2SubState) {
			case SLIDE_OFF:
				...
				if (x > PANEL_WIDTH) {
					...
					atk2Timer = ATK2_ARROW_SPAWN_DELAY;
				}
				break;
	...
	private void spawnAttack2Arrows() {
		double arrowSpeedY = atk2Top ? ATK2_ARROW_SPEED : -ATK2_ARROW_SPEED;
		
		double arrowStartY;
		if (atk2Top) {
			arrowStartY = y + DISPLAY_HEIGHT + 10;
		} else {
			arrowStartY = y - ATK2_ARROW_HEIGHT - 10;
		}
		
		int spacing = PANEL_WIDTH / 6;
		for (int i = 0; i < 5; i++) {
			double arrowX = spacing * (i + 1) - (ATK2_ARROW_WIDTH / 2.0);
